...
export default function Toast ({item, removeToast}) {
  useEffect(() => {
    setTimeout(() => removeToast(item.id), 10000);
  }, []);
  ...
  return(
    <div className="flex items-center p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
      {
        item.type == 'login' && <LoginTypeIcon/>
      }     
      {
        item.type == 'logout' && <LogoutTypeIcon/>
      }  
