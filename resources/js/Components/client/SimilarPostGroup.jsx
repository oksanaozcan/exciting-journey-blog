...
const SimilarPostGroup = ({similar_posts, isArticle=false}) => {
    ...
    <div className="p-10 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-5">
      {
        similarPosts.map(post => (
          <Link href={`/${isArticle ? 'articles' : 'posts'}/${post.id}`} className="rounded overflow-hidden shadow-lg" key={post.id}>
